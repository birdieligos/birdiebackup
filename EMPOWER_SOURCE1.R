...
meta <- meta %>%
  mutate(
    AD_TYPE = if_else(
      str_detect(PLACEMENT, regex("video", ignore_case = TRUE)),
      "Video",
      "Static"
    )
  )
...
meta <- meta %>%
  mutate(
    LANGUAGE = case_when(
      str_detect(AD_NAME, regex("English|ENG", ignore_case = TRUE)) ~ "English",
      str_detect(AD_NAME, regex("Spanish|SP", ignore_case = TRUE)) ~ "Spanish",
      TRUE                                                     ~ "Bilingual"
    )
  )
...
meta <- meta %>% mutate(
  Geography = case_when(
    str_detect(CAMPAIGN_NAME, regex("THMC", ignore_case=TRUE)) ~
      str_replace(
        str_extract(AD_GROUP_NAME, "Sector[0-9]+"),
        "([A-Za-z]+)([0-9]+)",
        "\\1 \\2"
      ),
    DATE <= as.Date("2024-12-30") ~ "Sector 1",
    TRUE ~ "Unspecified"
  ),
  Audience = case_when(
    str_detect(CAMPAIGN_NAME, regex("THMC", ignore_case=TRUE)) ~
      str_replace_all(
        str_extract(AD_GROUP_NAME, "(?<=Social_)[^_]+"),
        "([a-z])([A-Z])",
        "\\1 \\2"
      ),
    DATE <= as.Date("2024-12-30") ~ "Individual Automotive",
    TRUE ~ "Unspecified"
  )
)
...
tiktok <- select(tiktok, -VIDEO_VIEWS, -CURRENCY, -FORMAT)
tiktok$ACCOUNT_NAME <- "Thematic Campaigns"
...
demos_long <- demos %>%
  pivot_longer(
    cols = c(PHONENUMBER, WIRELESSPHONENUMBER),
    names_to = "PHONE_TYPE",
    values_to = "NUMBER_DIALED"
  ) %>%
  mutate(
    PHONE_TYPE = case_when(
      PHONE_TYPE == "PHONENUMBER" ~ "Landline",
      PHONE_TYPE == "WIRELESSPHONENUMBER" ~ "Mobile"
    )
  ) %>%
  filter(!is.na(NUMBER_DIALED) & NUMBER_DIALED != "")

print(colnames(demos))

demo_clean <- select(demos_long, LANGUAGE, V1_AGE, V1_ETHNICITY, V1_GENDER, V1_PDIID, RA_ZIP, GEOGRAPHY, 
                     CITYCODE, NUMBER_DIALED)

demo_clean <- demo_clean %>%
  mutate(CITYCODE = str_remove(CITYCODE, "^CA - "))


demo_clean <- select(demo_clean, -V1_PDIID)
...
demo_1 <- select(demo_clean, LANGUAGE, AD_GROUP_NAME, GEOGRAPHY, NUMBER_DIALED,
                 AUDIENCE)
...
demo_1 <- demo_1 %>%
  filter(!is.na(NUMBER_DIALED)) %>%
  distinct(NUMBER_DIALED, .keep_all = TRUE)
...
# report 
report <- outreach_merge

report <- report %>%
  filter(as.Date(DATE) >= as.Date("2025-01-10"))

report <- report %>%
  mutate(
    TYPE = case_when(
      PLATFORM %in% c("Convoso", "Hubdialer", "Impactive.io") ~ "Phones",
      PLATFORM == "PDI" ~ "Door to Door",
      PLATFORM == "Teletown Hall" ~ "Texts",
      TRUE ~ "Other"
    )
  )
...
report <- report %>%
  ungroup() %>%
  select(GEOGRAPHY, TYPE, IMPRESSIONS, REACH)

report <- select(report, GEOGRAPHY, TYPE, IMPRESSIONS, REACH)
...
write.csv(report_summary, file = '/Users/birdieligos/Documents/Reports/empower_outreach_bygeo070225.csv', row.names = FALSE)

print(unique(outreach_merge$PLATFORM))
################################################################
# Marketing bind
################################################################
### bind
