...
upload <- read.csv('/Users/sanjanalad/Downloads/GasparAllFlags_021126.csv', stringsAsFactors = FALSE)
...
bq_table <- bq_table(project = "slscampaigns-364520", dataset = "SignatureGathering_2026", table = "Gaspar_AllFlags")
bq_table_upload(bq_table, upload, write_disposition = "WRITE_TRUNCATE")


# Read Universe Contact List
universe <- read.csv('/Users/sanjanalad/Downloads/GasparUniverseContactList_021126.csv', stringsAsFactors = FALSE)
...
bq_table <- bq_table(project = "slscampaigns-364520", dataset = "SignatureGathering_2026", table = "Gaspar_Universe")
...
bq_table <- bq_table(project = "slscampaigns-364520", dataset = "SignatureGathering_2026", table = "Gaspar_SignatureData")
bq_table_upload(bq_table, joined, write_disposition = "WRITE_TRUNCATE")


# ---- VALIDATION CHECKS ----

# Check Gaspar_AllFlags table
tbl <- bq_table("slscampaigns-364520", "SignatureGathering_2026", "Gaspar_AllFlags")
allflags <- bq_table_download(tbl)
print(paste("AllFlags rows:", nrow(allflags)))
print(head(allflags))

# Check Gaspar_SignatureData table (the joined one)
tbl2 <- bq_table("slscampaigns-364520", "SignatureGathering_2026", "Gaspar_SignatureData")
